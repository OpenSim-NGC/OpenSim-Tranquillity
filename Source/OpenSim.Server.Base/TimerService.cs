...
public sealed class TimerService(ILogger<TimerService> logger) : IHostedService, IAsyncDisposable
{
    ...
    private void DoWork(object? state)
    {
        ...
        logger.LogInformation("{Service} is working, execution count: {Count:#,0}", nameof(TimerService), count);
    }

    public Task StopAsync(CancellationToken stoppingToken)
    {
        logger.LogInformation("{Service} is stopping.", nameof(TimerService));
